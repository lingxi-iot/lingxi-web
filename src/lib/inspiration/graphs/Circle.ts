...
import { AssetInfoPoint, GraphAnchor, GraphType } from '../common/render'
...
export class Circle extends BaseGraph {
      ...
      const anchorShape = new Konva.Line({
        ...
        stroke:
          adjustAnchor?.adjustType === anchor.adjustType && adjustAnchor?.groupId === graph.id()
            ? 'rgba(0,0,255,0.8)'
            : 'rgba(0,0,255,0.2)',
        strokeWidth: context.toStageValue(2),
        hitStrokeWidth: context.toStageValue(3),
        // 位置
        x,
        y,
        // 路径
  ...
  static override draw(graph: Konva.Group, context: RenderContext, adjustAnchor?: GraphAnchor) {
    // 调整点 及其 锚点
    const { anchorAndShadows } = super.draw(graph, context, adjustAnchor)

    return Circle.createAnchorShapes(context, graph, anchorAndShadows, adjustAnchor)
  }
