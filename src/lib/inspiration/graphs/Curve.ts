...
export class Curve extends BaseGraph {
  ...
  static override adjust(
    ...
    if (line && lineSnap) {
      ...
      // 更新 调整点（拐点）
      Curve.updateAnchor(context, graph)
  ...
  static override draw(graph: Konva.Group, context: RenderContext, adjustAnchor?: GraphAnchor) {
    ...
    return Curve.createAnchorShapes(context, graph, anchorAndShadows, adjustAnchor)
  }
  ...
  override drawEnd(): void {
    ...
    // 更新 调整点（拐点）
    Curve.updateAnchor(this.context, this.group)
