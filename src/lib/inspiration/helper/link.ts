...
import { BaseRender, LinkDrawPair, LinkDrawPoint, ManualPoint, ManualPointsMap, Render } from '../common/render'
...
export class Link extends BaseRender implements Render {
  ...
  constructor(context: RenderContext, layer: Konva.Layer, option: LinkOption) {
    super(context, layer)
    ...
    if (group) {
      ...
      if (this.context.debug) {
        const shape = new Konva.Rect({
          x: this.context.toStageValue(area.x1),
          y: this.context.toStageValue(area.y1),
          width: this.context.toStageValue(area.x2 - area.x1),
          height: this.context.toStageValue(area.y2 - area.y1),
  ...
  getGroupPairDistance(groupArea1: Area, groupArea2: Area): number {
    ...
    //
    return this.context.toBoardValue(
      Math.min(this.context.bgSize * 0.5, Math.max(dx < 6 ? 6 : dx, dy < 6 ? 6 : dy) * 0.5)
    )
  }
  ...
  changeLinkType(linkType: LinkType) {
    ...
    this.context.emit('link-type-change', this.state.linkType)
  }
  ...
  override draw() {
    ...
    // 所有层级的素材
    const groups = [
      ...(this.context.scene.find('.asset') as Konva.Group[]),
      ...(this.context.scene.find('.sub-asset') as Konva.Group[])
    ]
    ...
    if (this.context.debug && pairs.length > 0) {
      //console.log('')
      ...
      if (pair.from.groupId !== pair.to.groupId && !pair.disabled) {
        ...
        if (pair.linkType === LinkType.manual) {
          ...
          if (fromGroup && toGroup && fromPoint && toPoint) {
            ...
            // 拐点（已拐）记录
            const manualPointsMap: ManualPointsMap =
            ...
            const linkPoints = [
              [
                this.context.toStageValue(fromAnchorPos.x),
                this.context.toStageValue(fromAnchorPos.y)
              ],
              ...manualPoints.map((o) => [o.x, o.y]),
              [this.context.toStageValue(toAnchorPos.x), this.context.toStageValue(toAnchorPos.y)]
            ]
            ...
            for (let i = 0; i < linkPoints.length - 1; i++) {
              const circle = new Konva.Circle({
                ...
                radius: this.context.toStageValue(this.context.bgSize / 2),
                ...
                if (circle.attrs.dragStart) {
                  ...
                  if (pos) {
                    ...
                    tempPoints.splice(circle.attrs.linkManualIndex + 1, 0, [
                      this.context.toStageValue(transformerPos.x - stageState.x),
                      this.context.toStageValue(transformerPos.y - stageState.y)
                    ])
              ...
              if (this.context.debug) {
                for (const point of matrixPoints) {
                  this.group.add(
                    new Konva.Circle({
                      id: nanoid(),
                      x: this.context.toStageValue(point.x),
                      y: this.context.toStageValue(point.y),
                      radius: this.context.toStageValue(3),
                      stroke:
                        point.type === void 0
                          ? 'rgba(0,0,255,1)'
              ...
              if (matrixStart && matrixEnd) {
                ...
                const linkLine = new Konva.Line({
                  ...
                  points: _.flatten([
                    ...
                    ...way.map((o) => [
                      this.context.toStageValue(columns[o.x]),
                      this.context.toStageValue(rows[o.y])
                    ]),
                    [
                      this.context.toStageValue(toAnchorPos.x),
                      this.context.toStageValue(toAnchorPos.y)
                    ] // 补充 终点
                  ]),
                  stroke: 'red',
                  strokeWidth: 2
    ...
    for (const point of points) {
      ...
      if (group && !group.getAttr('selected')) {
        const anchor = this.context.scene.findOne(`#${point.id}`)
        ...
        if (anchor) {
          const circle = new Konva.Circle({
            ...
            id: point.id,
            groupId: group.id(),
            x: this.context.toStageValue(anchor.absolutePosition().x - stageState.x),
            y: this.context.toStageValue(anchor.absolutePosition().y - stageState.y),
            radius: this.context.toStageValue(this.option.size),
            stroke: 'rgba(255,0,0,0.2)',
            strokeWidth: this.context.toStageValue(1),
            // 调整中，不显示连接点
            opacity:
              point.visible &&
              !(this.context.draws[Helpers.Graph.name] as Helpers.Graph).state.adjusting
                ? 1
                : 0
          })
